...
- (id)initWithURL:(NSURL*)url
{
	...
	if ( result != noErr ) 
		NSLog(@"!!couldn't AudioSessionSetProperty( category ): OSStatus %ld", result);
	
	UInt32 one = 1;
	result = AudioSessionSetProperty( kAudioSessionProperty_OverrideCategoryMixWithOthers, sizeof(one), &one );
	if ( result != noErr )
		NSLog(@"!!couldn't AudioSessionSetProperty( override mix with others ): OSStatus %ld", result );
...
{	
	...
	{
		NSLog(@"asset reader is [%x] %@ %i", ( unsigned int )asset_reader, asset_reader, [asset_reader retainCount] );
		...
		else
		{
			...
			else
			{
				...
				NSLog(@"calling AudioQueueNewOutput with self %x", ( unsigned int )self );
	...
	if([self canPlay]) {
		...
		//	NSLog(@"playing, paused %c, audio_queue %x", paused?'y':'n', audio_queue );
			...
			if ( initialised && paused && audio_queue != NULL )
			{
				...
				if ( status != noErr )
				{
					NSLog(@"error calling AudioQueueStart: %ld", status );
				}
...
- (void)pause
{
	...
	//	NSLog(@"pausing, paused %c, audio_queue %x", paused?'y':'n', audio_queue );
		if ( !paused && audio_queue != nil )
		{
			...
			if ( status != noErr )
			{
				NSLog(@"error calling AudioQueuePause: %ld", status );
			}
		}
	}
#endif
...
- (void)dealloc
{
	...
	if ( audio_queue )
	{
		...
		if (status != noErr )
			NSLog(@"error calling AudioQueueStop: %ld", status);
	}
